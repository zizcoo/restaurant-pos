...
module.exports = async function handler(req, res) {
  ...
  try {
    await setupDB()
    const sql = getSQL()
    const rows = await sql`SELECT COALESCE(SUM(total),0) as total, COALESCE(SUM(CASE WHEN pay_method='CASH' THEN total ELSE 0 END),0) as cash, COALESCE(SUM(CASE WHEN pay_method='QR_PAY' THEN total ELSE 0 END),0) as qr, COUNT(*)::int as count FROM orders WHERE created_at::date = CURRENT_DATE AND status NOT IN ('rejected','pending')`
    res.json(rows[0])
