...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const existing = await sql`SELECT item_id FROM stock_out WHERE item_id = ${item_id}`
    if (existing.length > 0) { await sql`DELETE FROM stock_out WHERE item_id = ${item_id}` }
    else { await sql`INSERT INTO stock_out (item_id) VALUES (${item_id}) ON CONFLICT DO NOTHING` }
    const updated = await sql`SELECT item_id FROM stock_out`
    res.json(updated.map(r => r.item_id))
